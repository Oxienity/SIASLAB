...
pembobotanList.push({ 
  ...
  sertifikat,
  kompetensi,
  algo: g2.bobot, // nilaiKuliah adalah hasil dari algo+jar → g2
  jar: g2.bobot,  // bisa disamakan, atau buat terpisah kalau ingin lebih rinci
  suara: g5.bobot,
  sikap: g6.bobot,
  interaksi: g7.bobot,
  penguasaan: g8.bobot,
  penyampaian: g9.bobot,
  komitmen: g10.bobot,
  konsistensi: g11.bobot,
  karakter: g12.bobot
});
...
function tampilkanGAP() {
  ...
  console.log("Fungsi tampilkanGAP dipanggil");
...
function tampilkanPembobotanNilai() {
  ['bobotAdminTable', 'bobotKompTable', 'bobotMicroTable', 'bobotWawanTable'].forEach(id => {
    ...
    console.log("Fungsi tampilkanPembobotanNilai dipanggil");
  });
  ...
  pembobotanList.forEach(p => {
    ...
    const kompRow = `<tr><td>${p.nama}</td><td>${p.kompetensi}</td><td>${p.nilaiKuliah}</td></tr>`;
...
function showSection(sectionId, linkElement) {
  ...
  // Tampilkan tabel GAP atau Pembobotan jika diperlukan
  if (sectionId === 'gapSection') {
    tampilkanGAP();
    let html = `<div class="mb-4"><h4 class="judul-section">${aspek.nama}</h4>`;
  } else if (sectionId === 'bobotSection') {
    tampilkanPembobotanNilai();
  }
}

// Saat halaman dimuat, langsung tampilkan section Team
window.addEventListener("DOMContentLoaded", () => {
  // Panggil showSection untuk Team
  showSection('team', document.querySelector('.nav-link[onclick*="team"]'));
  
  // Jika ada fungsi muatDataAwal, jalankan
  if (typeof muatDataAwal === 'function') muatDataAwal();
});
